...
struct playing_time_info {
    ...
    /* kibibytes played before and after current pos, and total.
       Kibibytes because bytes would overflow. Data type range is up
       to 2TB. */
                                                   ...
                                                   bool say_it)
{
    ...
    case 5: { /* storage size */
        ...
        output_dyn_value(str1, sizeof(str1), pti->kbs_ttl, kbyte_units, 3, true);
        output_dyn_value(str2, sizeof(str2), pti->kbs_bef, kbyte_units, 3, true);
        output_dyn_value(str3, sizeof(str3), pti->kbs_aft, kbyte_units, 3, true);
        snprintf(buf, buffer_len, str(LANG_PLAYTIME_STORAGE),
                 str1,str2,str3);
        if(say_it) {
            talk_id(LANG_PLAYTIME_STORAGE, false);
            output_dyn_value(NULL, 0, pti->kbs_ttl, kbyte_units, 3, true);
            talk_ids(true, VOICE_PAUSE, VOICE_PLAYTIME_DONE);
            output_dyn_value(NULL, 0, pti->kbs_bef, kbyte_units, 3, true);
            talk_id(LANG_PLAYTIME_REMAINING, true);
            output_dyn_value(NULL, 0, pti->kbs_aft, kbyte_units, 3, true);
        }
        break;
    }
    case 6: { /* Average track file size */
        ...
        output_dyn_value(str, sizeof(str), avg_track_size, kbyte_units, 3, true);
        snprintf(buf, buffer_len, str(LANG_PLAYTIME_AVG_TRACK_SIZE),
                 str);
        if(say_it) {
            talk_id(LANG_PLAYTIME_AVG_TRACK_SIZE, false);
            output_dyn_value(NULL, 0, avg_track_size, kbyte_units, 3, true);
        }
        break;
    }
