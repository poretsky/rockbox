...
{
    if ((unsigned int)req->buffer & 0x3) /* not word aligned ? */
...
static int sd_init_device(const int drive)
{
    ...
    do retval = sd_init_card_state(drive, &init_req);
...
int sd_transfer_sectors(IF_MD(int drive,) unsigned long start, int count, void* buf, bool write)
{
    struct sd_request request;
    struct sd_response_r1 r1;
    int retval = -1;
...
static int sd1_oneshot_callback(struct timeout *tmo)
{
    ...
    queue_broadcast(state ? SYS_HOTSWAP_INSERTED : SYS_HOTSWAP_EXTRACTED, sd_drive_nr + SD_SLOT_1);
...
static int sd2_oneshot_callback(struct timeout *tmo)
{
    ...
    /* This is called only if the state was stable for 300ms - check state
     * and post appropriate event. */
    queue_broadcast(state ? SYS_HOTSWAP_INSERTED : SYS_HOTSWAP_EXTRACTED, sd_drive_nr + SD_SLOT_2);
